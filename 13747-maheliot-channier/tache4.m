...
% préambule + retard + décallage freq (multiplication tous le signal par
% exp)


% rajouter 20*dt 0 à yl au début pour représenter le retard
...
%{
for q=1:length(retard2)
    if retard2(q)==0
        sl(1+(q-1)*20:20+(q-1)*20)=po;
    end
    
    if retard2(k)==1
        sl(1+(q-1)*20:20+(q-1)*20)=p1;
    end
end
%}
...
dtmax = synchro(rld2,preambule,Te,Tp,Fse);
disp("La valeur de dt estimée est de :");
disp(dtmax);
disp("La valeur de dt initial est de :")
disp(val_dt);
...
ylabel('PEB')
xlabel('E_b/N_0 (dB)')
title('Comparaison PEB théorique/expérimentale tâche 4')
