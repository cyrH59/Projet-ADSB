function [registre] = bit2registre_V2(b,latref,lonref)
...
'altitude',[],'timeFlag',[],'cprFlag',[],'latitude',[],'longitude',[],'vitesse',[]);
     format = trame(1:5);
     format_val = 0;
     ...
     elseif type_val == 19
         ...
         subtype = trame(38:40);
         subtypegood = transpose([0 1 1]);
         subtypegood2 = transpose([1 0 0]);
         if subtype == subtypegood
             disp("We can get the normal air speed here");
             speed = trame(58:67);
             speed_val = 0;
             for k=1:length(speed)
                 if speed(k)==1
                     speed_val = speed_val + 2^(length(speed)-k);
                 end
             end
             registre.vitesse = speed_val;
         end
         if subtype == subtypegood2
             disp("We can get the supersonic air speed here");
             speed = trame(58:67);
             speed_val = 0;
             for k=1:length(speed)
                 if speed(k)==1
